...
We use the statistical programming language `R` [@citeR]. In the data analysis and visualization process, we also made use of the following `R` packages: `readr` [@citereadr], `dplyr` [@citedplyr], `knitr` [@citeknitr], `kableExtra` [@citekExtra], `ggplot2` [@citeggplot2], `stringr` [@citestringr], `janitor` [@citejanitor], `rstanarm` [@citerstanarm], `modelsummary` [@citemodelsummary], `broom` [@citebroom], `broom.mixed` [@citebroomMix], `parameters` [@citeparameters], `magrittr` [@citemagrittr], `tidyverse` [@citetidyverse], `patchwork` [@citepatchwork], `bayesplot` [@citebayesplot], and `loo` [@citeloo].
...
To make it simpler to create a linear model to analyze the death data, we selected the top 10 causes of death for the year 2022. To present this data more directly, we created tables and figures. @tbl-topcauses-3 shows the Top 10 causes of death in Alberta in 2022.
...
We then created line graphs of the top five causes of death in 2022, showing how they performed from 2001 to 2022 (@fig-topcauses-4).

```{r}
#| echo: false
#| warning: false
#| message: false
#| lable: "fig-topcauses-4"
...
The Negative Binomial model demonstrated a better fit over the Poisson model as indicated by the Leave-One-Out (LOO) Cross-Validation results. In this case, we find that the Negative Binomial model is a better fit than the Poisson because ELPD is larger.
